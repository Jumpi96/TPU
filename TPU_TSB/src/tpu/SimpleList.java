...
public class SimpleList <E extends Comparable> implements Iterable<E>
{
      ...
      public boolean containsContenido( String x )
      {
          Node <E> p = frente;
          while ( p != null && x.compareTo( ((Palabra)p.getInfo()).getContenido() ) != 0 ) { p = p.getNext(); }    
          return ( p != null );
      }
      
      public E searchContenido( String x )
      {
          for ( Node <E> p = frente; p != null; p = p.getNext() )
            {
                if( x.compareTo(((Palabra)p.getInfo()).getContenido()) == 0 )
                    { return p.getInfo(); }
            }
            return null;
      }
